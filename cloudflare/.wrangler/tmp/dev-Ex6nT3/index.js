...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    if (isApi("admin/dev-seed") && request.method === "POST") {
      ...
      if (!reqs.length && !sups.length && !dems.length) {
        const base2 = url.searchParams.get("base") || "http://127.0.0.1:5500/data";
        reqs = await fetchJsonSafe(`${base2}/requirements.json`);
        sups = await fetchJsonSafe(`${base2}/suppliers.json`);
        dems = await fetchJsonSafe(`${base2}/demanders.json`);
      }
      const now = (/* @__PURE__ */ new Date()).toISOString();
      for (const r of reqs || []) {
        try {
    ...
    if (isApi("admin/import-suppliers") && request.method === "POST") {
      ...
      try {
        const base2 = url.searchParams.get("base") || "http://127.0.0.1:5500/data";
        let sups = await fetchJsonSafe(`${base2}/suppliers.json`);
        const now = (/* @__PURE__ */ new Date()).toISOString();
        if (!Array.isArray(sups) || !sups.length) {
          const siteBase = String(base2).replace(/\/?data\/?$/i, "");
          const idx = await fetchJsonSafe(`${siteBase}/index.json`);
          const list = Array.isArray(idx) ? idx : [];
          const items = list.filter((i) => {
            const t = String(i.type || i.section || "").toLowerCase();
  ...
  async scheduled(event, env, ctx) {
    const base2 = env.SYNC_BASE_URL || "https://www.visndt.com/data";
    try {
      const reqs = await fetch(`${base2}/requirements.json`).then((r) => r.json()).catch(() => []);
      let sups = await fetch(`${base2}/suppliers.json`).then((r) => r.json()).catch(() => []);
      const dems = await fetch(`${base2}/demanders.json`).then((r) => r.json()).catch(() => []);
      const now = (/* @__PURE__ */ new Date()).toISOString();
      for (const r of reqs || []) {
        try {
          await env.DB.prepare(`INSERT OR IGNORE INTO requirements (
...
// .wrangler/tmp/bundle-o192ZS/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-o192ZS/middleware-loader.entry.ts

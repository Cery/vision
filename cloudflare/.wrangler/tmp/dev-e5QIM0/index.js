  ...
  async fetch(request, env, ctx) {
    ...
    if (isApi("products") && request.method === "GET" && !path.startsWith("/api/admin/products")) {
      ...
      const items = (results || []).map((p) => {
        let params = {};
        try {
          params = JSON.parse(p.parameters_json || "{}");
        } catch {
        }
        const norm = /* @__PURE__ */ __name((k) => String(k || "").toLowerCase(), "norm");
        const getVal = /* @__PURE__ */ __name((key) => {
    ...
    if (isApi("markets") && request.method === "GET") {
      ...
      const countBinds = [];
      if (progress) {
        baseSql += " AND progress = ?";
        binds.push(progress);
        countBinds.push(progress);
      }
      if (category) {
        baseSql += " AND primary_category = ?";
        binds.push(category);
    ...
    if (isApi("suppliers/register") && request.method === "POST") {
      try {
        ...
        const now = (/* @__PURE__ */ new Date()).toISOString();
        const supplier_id = "SUP-" + now.replace(/[-:T.Z]/g, "") + "-" + Math.floor(Math.random() * 1e3);
        ...
        await env.DB.prepare("INSERT INTO suppliers (supplier_id, name, company, access_password_plain, contact_phone, contact_email, status, metadata_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(supplier_id, name, company, access_password_plain, contact_phone, contact_email, "pending", JSON.stringify(metadata), now, now).run();
    ...
    if (isApi("admin/suppliers/") && /\/ingest-gallery$/.test(path) && request.method === "POST") {
      ...
      const isDataUrl = /* @__PURE__ */ __name((s) => typeof s === "string" && /^data:image\//i.test(s), "isDataUrl");
      const toBytes = /* @__PURE__ */ __name((dataUrl) => {
      ...
      const labelFor = /* @__PURE__ */ __name((idx) => {
        const m = (labels[idx] || "").trim();
        if (m) return m;
        return ["\u95E8\u5934", "\u5DE5\u5382", "\u8F66\u95F4"][idx] || `\u56FE\u7247${idx + 1}`;
      }, "labelFor");
      const slug = /* @__PURE__ */ __name((s) => String(s || "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "image", "slug");
      const extFor = /* @__PURE__ */ __name((ct) => /jpeg/i.test(ct) ? "jpg" : /png/i.test(ct) ? "png" : "bin", "extFor");
      const uploaded = [];
      ...
      await env.DB.prepare("UPDATE suppliers SET metadata_json = ?, updated_at = ? WHERE supplier_id = ?").bind(JSON.stringify(newMeta), (/* @__PURE__ */ new Date()).toISOString(), supplier_id).run();
    ...
    if (isApi("admin/suppliers/ingest-gallery-bulk") && request.method === "POST") {
      ...
      for (const sid of ids) {
        try {
          ...
          if (!row) {
            out.push({ supplier_id: sid, ok: false, error: "NotFound" });
            continue;
          }
          const meta = parseJSONSafe(row.metadata_json) || {};
          const imgs = Array.isArray(meta.gallery_images) ? meta.gallery_images : [];
          ...
          if (uploaded.length) {
            ...
            await env.DB.prepare("UPDATE suppliers SET metadata_json = ?, updated_at = ? WHERE supplier_id = ?").bind(JSON.stringify(newMeta), (/* @__PURE__ */ new Date()).toISOString(), sid).run();
          }
    ...
    if (isApi("admin/products") && request.method === "GET") {
      ...
      if (possibleId && possibleId !== "products") {
        const { results: results2 } = await env.DB.prepare("SELECT * FROM products WHERE product_id = ? OR slug = ?").bind(possibleId, possibleId).all();
      ...
      if (where.length) q += " WHERE " + where.join(" AND ");
      const limitAdminProd = Math.min(parseInt(url.searchParams.get("limit") || "100", 10) || 100, 1e3);
      q += " ORDER BY created_at DESC LIMIT ?";
    ...
    if (isApi("admin/news") && request.method === "GET") {
      ...
      try {
        ...
        if (possibleId && possibleId !== "news") {
          const { results: results2 } = await env.DB.prepare("SELECT * FROM news WHERE news_id = ? OR id = ? OR slug = ?").bind(possibleId, possibleId, possibleId).all();
          if (!results2 || !results2.length) return json({ error: "NotFound" }, 404);
          return json(results2[0]);
        }
        const limitAdminNews = Math.min(parseInt(url.searchParams.get("limit") || "100", 10) || 100, 1e3);
    ...
    if (isApi("admin/cases") && request.method === "GET") {
      ...
      try {
        ...
        if (possibleId && possibleId !== "cases") {
          const { results: results2 } = await env.DB.prepare("SELECT * FROM cases WHERE case_id = ? OR slug = ?").bind(possibleId, possibleId).all();
          if (!results2 || !results2.length) return json({ error: "NotFound" }, 404);
          return json(results2[0]);
        }
        const limitAdminCases = Math.min(parseInt(url.searchParams.get("limit") || "100", 10) || 100, 1e3);
    ...
    if (isApi("admin/import-news") && request.method === "POST") {
      ...
      try {
        ...
        try {
          ...
          let prev = {};
          try {
            const r = await env.DB.prepare("SELECT value_json FROM system_config WHERE key = ?").bind("sync_meta").first();
            prev = r && r.value_json ? JSON.parse(r.value_json || "{}") : {};
          } catch {
          }
          prev.base = base;
    ...
    if (isApi("admin/products") && request.method === "POST") {
      ...
      const data = await bodyJSON(request);
      const now = (/* @__PURE__ */ new Date()).toISOString();
      const product_id = data.ProductID || data.product_id || "PROD-" + now.replace(/[-:T.Z]/g, "") + "-" + Math.floor(Math.random() * 1e4);
...
// .wrangler/tmp/bundle-fWzVHW/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-fWzVHW/middleware-loader.entry.ts

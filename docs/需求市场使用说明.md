...
- 提交到 `POST /api/requirements`，成功返回：
  - 需求ID（显示并提供复制按钮）
  - 需求方管理密码（显示并提供复制按钮）
- 快捷按钮：进入入口页（携带 `id`）
...
- 加载：`GET /api/requirements/:id`，携带 `password` 或 `supplier_access_password`。
- 视图差异：
...
- 列表：`GET /api/requirements`（公开列表，受审核且状态为公开）
...
- `POST /api/requirements` 创建需求（cloudflare/src/index.js:195-244）
  - 请求体示例：
    ```json
    {
      "Title": "电子内窥镜采购",
...
- `GET /api/requirements` 列表（cloudflare/src/index.js:246-269）
...
- `GET /api/requirements/:id` 详情（cloudflare/src/index.js:271-336）
...
- 管理更新单条（cloudflare/src/index.js:339-401）
  - `PATCH /api/requirements/:id`（管理员或需求方）
...
```mermaid
flowchart LR
  A[publish.html 提交] --> B[POST /api/requirements]
  B -->|返回 RequirementID/密码| C[entry.html 输入 id + 密码]
  C --> D[view.html 加载 GET /api/requirements/:id]
  D --> E{role}
...
- 获取公开列表
```bash
curl 'http://localhost:8000/api/requirements?category=%E7%94%B5%E5%AD%90%E5%86%85%E7%AA%A5%E9%95%9C'
```

- 详情（携带密码）
```bash
curl 'http://localhost:8000/api/requirements/REQ-20250101-1234?password=123456&supplier_access_password=sup-abc'
```
...
- 管理员更新需求密码
```bash
curl -s -X PATCH 'http://localhost:8000/api/requirements/REQ-20250101-1234' \
  -H 'Content-Type: application/json' \

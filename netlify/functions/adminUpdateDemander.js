...
exports.handler = async function(event){
  ...
  const { id, CompanyName, Title, ContactName, Phone, Email, ContactPublic, Password } = payload;
  const fields = {
  ...
  try{
    if (ok){
      ...
      const list = await readJson('data/demanders.json', []);
      ...
      await writeJson('data/demanders.json', list);
      const rec = list[foundIdx >= 0 ? foundIdx : (list.length - 1)];
      return okJson({ id: rec.id, fields: rec });
    }
  }catch(e){
    return errJson(500, 'UpdateDemanderFailed', e.message);
  }
};